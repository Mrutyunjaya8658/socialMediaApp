...
const CreatePost = ({ open, setOpen }) => {
  ...
  const createPostHandler = async (e) => {
    ...
    if (!caption && !imagePreview) {
      toast.error("Please provide a caption or an image");
      return;
    }
  ...
  return (
    <Dialog open={open}>
      <DialogContent onInteractOutside={() => setOpen(false)}>
        ...
        <Button onClick={() => imageref.current.click()} className="w-full my-4">
          Choose Image
        </Button>
        ...
        {imagePreview || caption ? (
          loading ? (
            ...
            <Button onClick={createPostHandler} className="w-full" disabled={loading}>
              Post
            </Button>
          )
        ) : (
          <Button disabled className="w-full">
            Post (Provide a caption or image)
          </Button>
        )}
      </DialogContent>
    </Dialog>
